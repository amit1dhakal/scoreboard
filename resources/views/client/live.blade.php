                ...
                <div class="col-md-12 col-lg-6 col-sm-12">
                    <div class="card">
                        <div class="card-header scoreboardheader">
                            <div> Score Board </div>
                            <div id="matchStatusUpdate_{{ $match->slug }}" style="font-size: 20px;"> @include('admin.include.matchstatus') </div>
                        </div>
                        ...
                        <div class="card-footer displayMatchFooter" id="lastEventDisplay_{{$match->slug}}">
                            <p>@isset($match->allgoalfoul->first()->player)
                                {{ 'Last '. $match->allgoalfoul->first()->type.' by '. $match->allgoalfoul->first()->player->name}} ({{$match->allgoalfoul->first()->player->jersey_no}}) at {{$match->allgoalfoul->first()->event_time}} minutes - {{$match->allgoalfoul->first()->team->name}}
                                @endisset 
                            </p>
                        </div>
                    </div>
