    ...
    <main>
        <div class="container">
            <div class="card">
                ...
                <form action="{{ route('match.update', $match->slug) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                                    ...
                                    <select class="form-control" name="team1_id" id="team1_id" required>
                                        @foreach ($teams as $team)
                                            <option value="{{ $team->id }}"
                                                @if ($match->team_ids[0] == $team->id) selected @endif>{{ $team->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6 col-sm-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="team2_id" id="team2_id" required>
                                        @foreach ($teams as $team)
                                            <option value="{{ $team->id }}"
                                                @if ($match->team_ids[1] == $team->id) selected @endif>{{ $team->name }}
                                            </option>
                                        @endforeach
                                    </select>
