    ...
    <main>
        <div class="container">
            <div class="card">
                ...
                <form action="{{ route('match.update', $match->slug) }}" method="post" onsubmit=" return checkFieldSide()">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                                    ...
                                    <select class="form-control" name="home_team_id" id="home_team_id" required>
                                        @foreach ($teams as $team)
                                            <option value="{{ $team->id }}"
                                                @if ($match->home_team_id== $team->id) selected @endif>{{ $team->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6 col-sm-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="away_team_id" id="away_team_id" required>
                                        @foreach ($teams as $team)
                                            <option value="{{ $team->id }}"
                                                @if ($match->away_team_id == $team->id) selected @endif>{{ $team->name }}
                                            </option>
                                        @endforeach
                                    </select>
